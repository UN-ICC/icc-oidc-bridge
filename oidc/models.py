...
class PresentationConfigurations(models.Model):
    ...
    def to_json(self):
        ...
        if "non_revoked" in self.configuration:
            presentation_request["non_revoked"] = self.configuration.get("non_revoked")
            if not "to" in presentation_request["non_revoked"]:
                presentation_request["non_revoked"].update({"to": int(timezone.now().timestamp()) + 300})
